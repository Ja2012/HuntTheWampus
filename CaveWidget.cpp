...
void CbChangeLabelColor(Fl_Widget* Widget)
{
	if (Widget->labelcolor() == FL_DARK_BLUE) Widget->labelcolor(FL_RED);
	else if (Widget->labelcolor() == FL_RED) Widget->labelcolor(FL_BLACK);
	else if (Widget->labelcolor() == FL_BLACK) Widget->labelcolor(FL_DARK_GREEN);
	else if (Widget->labelcolor() == FL_DARK_GREEN) Widget->labelcolor(FL_DARK_BLUE);
	Widget->redraw();
}
...
int CaveWidget::handle(int Event)
{
	if (Event == FL_PUSH)
	{
		do_callback();
